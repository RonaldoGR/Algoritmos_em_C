...
int main(void) {
	...
	do {
		...
		printf("\t11 --->\t Grava em Arquivo de Texto\n");
		printf("\t12 --->\t Lê de Arquivo de Texto\n");
		printf("+-----------------------------------------+\n");
		printf("\n\n");
		...
		switch (op) {
			...
			case 8: {
				...
				int resultado = consultaPorCodigo(&lista, codigo, &dadoConsultado);
			...
			case 9: {
				...
				if(incluiDepois(&lista, codigo, dado) == CODIGO_INEXISTENTE) {
					printf("Código inexistente.\n");
					break;
				}
			...
			case 12: {
				Dado dado;
				if(leituraDeDados(&lista, dado) == FALHOU){
					printf ("Nao foi possivel abrir o arquivo \n");
					break;
				}
